    ...
    for (i=is; i<=ie; i++) {
      ...
      for (n=0; n<NSCALARS; n++)
        ...
        if (list->Rmass > 0) {
            ...
            if (dtodx1*rsf*x1Flux[j][i+1].d > 0) {
                /* calculate probabiliy for mc tracer transfer */
                pflux = (dtodx1*rsf*x1Flux[j][i+1].d)/(list->Rmass);
                prob_iterate_x1(list, pflux, pG);
                /* Update reduced mass */
                list->Rmass -= (dtodx1*rsf*x1Flux[j][i+1].d);
            }
            /* If outgoing (left face) flux, MC iterate through tracers */
            if (dtodx1*lsf*x1Flux[j][i].d < 0) {
                /* calculate probabiliy for mc tracer transfer */
                pflux = (dtodx1*lsf*x1Flux[j][i].d)/(list->Rmass);
                prob_iterate_x1(list, pflux, pG);
                /* Update reduced mass */
                list->Rmass += (dtodx1*lsf*x1Flux[j][i].d);
            }
        }
#endif
    }
      ...
      for (n=0; n<NSCALARS; n++)
        ...
        if (list->Rmass > 0) {
            ...
            if (dtodx2*rsf*x2Flux[j+1][i].d > 0) {
                /* calculate probabiliy for mc tracer transfer */
                pflux = (dtodx2*rsf*x2Flux[j+1][i].d)/(list->Rmass);
                prob_iterate_x2(list, pflux, pG);
                /* Update reduced mass */
                list->Rmass -= (dtodx2*rsf*x2Flux[j+1][i].d);
            }
            /* If outgoing (left face) flux, MC iterate through tracers */
            if (dtodx2*lsf*x2Flux[j][i].d < 0) {
                /* calculate probabiliy for mc tracer transfer */
                pflux = (dtodx2*lsf*x2Flux[j][i].d)/(list->Rmass);
                prob_iterate_x2(list, pflux, pG);
                /* Update reduced mass */
