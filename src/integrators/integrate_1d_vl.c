...
#ifdef MCTRACERS
      list = &((pG->GridLists)[ks][js][i]);
      if (list->Rmass > 0) {
          /* If outgoing (right face) flux, iterate through tracers */
          if (dtodx1*rsf*x1Flux[i+1].d > 0) {
              /* calculate probabiliy for tracer transfer */
              pflux = (dtodx1*rsf*x1Flux[i+1].d)/(list->Rmass);
              prob_iterate_x1(list, pflux, pG);
              /* Update reduced mass */
              list->Rmass -= (dtodx1*rsf*x1Flux[i+1].d);
          }
          /* If outgoing (left face) flux, iterate through tracers */
          if (dtodx1*lsf*x1Flux[i].d < 0) {
              /* calculate probabiliy for tracer transfer */
              pflux = (dtodx1*lsf*x1Flux[i].d)/(list->Rmass);
              prob_iterate_x1(list, pflux, pG);
              /* Update reduced mass */
              list->Rmass += (dtodx1*rsf*x1Flux[i].d);
          }
      }
#endif
  }
...
/* x1-boundaries of parent Grids (at boundaries of THIS Grid)  */
  for (npg=0; npg<pG->NPGrid; npg++) {
    ...
    /*--- Step 13e -----------------------------------------------------------------
     * Sweep through tracer lists and move tracers marked for transfer
     */
