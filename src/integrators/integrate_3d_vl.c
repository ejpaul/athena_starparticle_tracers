 ...
 */
      ...
      lr_states(pG,W1d,Bxc,pG->dt,pG->dx1,is-ngsp,ie+ngsp,Wl,Wr,1);

      for (i=is-ngsp; i<=ie+ngsp+1; i++) {
 ...
 */
      ...
      dx2 = r[i]*pG->dx2;
#endif
      lr_states(pG,W1d,Bxc,pG->dt,dx2,js-ngsp,je+ngsp,Wl,Wr,2);

      for (j=js-ngsp; j<=je+ngsp+1; j++) {
 ...
 */
      ...
      lr_states(pG,W1d,Bxc,pG->dt,pG->dx3,ks-ngsp,ke+ngsp,Wl,Wr,3);

      for (k=ks-ngsp; k<=ke+ngsp+1; k++) {
...
/* revert to predictor flux if zone velocity is too high */
#ifdef MACH_NUMBER_CHECK
        if((SQR(Wl_x1Face[k][j][i].Vx) > (MSQR*Wl_x1Face[k][j][i].P/Wl_x1Face[k][j][i].d)) ||
           (SQR(Wl_x1Face[k][j][i].Vx) > (MSQR*Wr_x1Face[k][j][i].P/Wr_x1Face[k][j][i].d)) ||
           (SQR(Wr_x1Face[k][j][i].Vx) > (MSQR*Wl_x1Face[k][j][i].P/Wl_x1Face[k][j][i].d)) ||
           (SQR(Wr_x1Face[k][j][i].Vx) > (MSQR*Wr_x1Face[k][j][i].P/Wr_x1Face[k][j][i].d))){
          x1Flux[k][j][i] = x1FluxP[k][j][i];
          NaNFlux++;
        }
#endif
...
/* revert to predictor flux if zone velocity is too high */
#ifdef MACH_NUMBER_CHECK
        if((SQR(Wl_x2Face[k][j][i].Vx) > (MSQR*Wl_x2Face[k][j][i].P/Wl_x2Face[k][j][i].d)) ||
           (SQR(Wl_x2Face[k][j][i].Vx) > (MSQR*Wr_x2Face[k][j][i].P/Wr_x2Face[k][j][i].d)) ||
           (SQR(Wr_x2Face[k][j][i].Vx) > (MSQR*Wl_x2Face[k][j][i].P/Wl_x2Face[k][j][i].d)) ||
           (SQR(Wr_x2Face[k][j][i].Vx) > (MSQR*Wr_x2Face[k][j][i].P/Wr_x2Face[k][j][i].d))){
          x2Flux[k][j][i] = x2FluxP[k][j][i];
          NaNFlux++;
        }
#endif
...
/* revert to predictor flux if zone velocity is too high */
#ifdef MACH_NUMBER_CHECK
        if((SQR(Wl_x3Face[k][j][i].Vx) > (MSQR*Wl_x3Face[k][j][i].P/Wl_x3Face[k][j][i].d)) ||
           (SQR(Wl_x3Face[k][j][i].Vx) > (MSQR*Wr_x3Face[k][j][i].P/Wr_x3Face[k][j][i].d)) ||
           (SQR(Wr_x3Face[k][j][i].Vx) > (MSQR*Wl_x3Face[k][j][i].P/Wl_x3Face[k][j][i].d)) ||
           (SQR(Wr_x3Face[k][j][i].Vx) > (MSQR*Wr_x3Face[k][j][i].P/Wr_x3Face[k][j][i].d))){
          x3Flux[k][j][i] = x3FluxP[k][j][i];
          NaNFlux++;
        }
#endif
...
#ifdef FIRST_ORDER_FLUX_CORRECTION
  if (NaNFlux != 0) {
//    ath_perr(-1,"[Step10] %i second-order fluxes replaced\n",NaNFlux);
    NaNFlux=0;
  }
#endif
        ...
        for (n=0; n<NSCALARS; n++)
          ...
          if (list->Rmass > 0) {
              if (dtodx1*rsf*x1Flux[k][j][i+1].d > 0) {
                  /* calculate probabiliy for mc tracer transfer */
                  pflux = (dtodx1*rsf*x1Flux[k][j][i+1].d)/(list->Rmass);
                  prob_iterate_x1(list, pflux, pG);
                  /* Update reduced mass */
                  list->Rmass -= (dtodx1*rsf*x1Flux[k][j][i+1].d);
              }
              /* If outgoing (left face) flux, MC iterate through tracers */
              if (dtodx1*lsf*x1Flux[k][j][i].d < 0) {
                  /* calculate probabiliy for mc tracer transfer */
                  pflux = (dtodx1*lsf*x1Flux[k][j][i].d)/(list->Rmass);
                  prob_iterate_x1(list, pflux, pG);
                  /* Update reduced mass */
                  list->Rmass += (dtodx1*lsf*x1Flux[k][j][i].d);
              }
          }
        ...
        for (n=0; n<NSCALARS; n++)
          ...
          if (list->Rmass > 0) {
              if (dtodx2*rsf*x2Flux[k][j+1][i].d > 0) {
                  /* calculate probabiliy for mc tracer transfer */
                  pflux = (dtodx2*rsf*x2Flux[k][j+1][i].d)/(list->Rmass);
                  prob_iterate_x2(list, pflux, pG);
                  /* Update reduced mass */
                  list->Rmass -= (dtodx2*rsf*x2Flux[k][j+1][i].d);
              }
              /* If outgoing (left face) flux, MC iterate through tracers */
              if (dtodx2*lsf*x2Flux[k][j][i].d < 0) {
                  /* calculate probabiliy for mc tracer transfer */
                  pflux = (dtodx2*lsf*x2Flux[k][j][i].d)/(list->Rmass);
                  prob_iterate_x2(list, pflux, pG);
                  /* Update reduced mass */
                  list->Rmass += (dtodx2*lsf*x2Flux[k][j][i].d);
              }
          }
#endif /* MCTRACERS */
      }
        ...
        for (n=0; n<NSCALARS; n++)
          ...
          if (list->Rmass > 0) {
              ...
              if (dtodx3*rsf*x3Flux[k+1][j][i].d > 0) {
                  /* calculate probabiliy for mc tracer transfer */
                  pflux = (dtodx3*rsf*x3Flux[k+1][j][i].d)/(list->Rmass);
                  prob_iterate_x3(list, pflux, pG);
                  /* Update reduced mass */
                  list->Rmass -= (dtodx3*rsf*x3Flux[k+1][j][i].d);
              }
              /* If outgoing (left face) flux, MC iterate through tracers */
              if (dtodx3*lsf*x3Flux[k][j][i].d < 0) {
                  /* calculate probabiliy for mc tracer transfer */
                  pflux = (dtodx3*lsf*x3Flux[k][j][i].d)/(list->Rmass);
                  prob_iterate_x3(list, pflux, pG);
                  /* Update reduced mass */
                  list->Rmass += (dtodx3*lsf*x3Flux[k][j][i].d);
              }
          }
#endif /* MCTRACERS */
      }
    ...
    for (j=js; j<=je; j++) {
      for (i=is; i<=ie; i++) {
        ...
        if (W.d < 0.0) {
          ...
          BadCell.k = k;
          negd++;
//          ath_perr(-1,"[Step14]: at t=%g; d<0; n=%g, P=%g\n",pG->time,W.d, W.P);
        }
#ifndef ISOTHERMAL
        if (W.P/W.d < 0.06) {
          flag_cell = 1;
          ...
          BadCell.k = k;
          negP++;
//          ath_perr(-1,"[Step14]: at t=%g; T<10; n=%g, P=%g\n",pG->time,W.d, W.P);
        }
#endif
        if (flag_cell != 0) {
          FixCell(pG, BadCell);
          flag_cell=0;
//          W = Cons_to_Prim(&(pG->U[k][j][i]));
//          ath_perr(-1,"[Step14]: After FOFC; n=%g, P=%g\n",pG->time,W.d,W.P);
        }
      }
    }
  }
  ...
  if (negd > 0 || negP > 0)
    ath_perr(-1,"[Step14]: at t=%g; %i cells had d<0; %i cells had P<0\n",pG->time,negd,negP);
#endif /* FIRST_ORDER_FLUX_CORRECTION */
...
#ifdef STAR_PARTICLE
  update_starparticles(pG,x1Flux,x2Flux,x3Flux);
#endif
  ...
  for (n=0; n<NSCALARS; n++)
    ...
    if (list->Rmass > 0) {
        if (dtodx1*rsf*rx1*x1FD_ip1.d < 0) {
            /* calculate probabiliy for mc tracer transfer */
            pflux = (dtodx1*rsf*rx1*x1FD_ip1.d)/(list->Rmass);
            prob_iterate_x1(list, pflux, pG);
            /* Update reduced mass */
            list->Rmass += dtodx1*rsf*rx1*x1FD_ip1.d;
        }
        /* If outgoing (left face) flux, MC iterate through tracers */
        if (dtodx1*lsf*lx1*x1FD_i.d > 0) {
            /* calculate probabiliy for mc tracer transfer */
            pflux = (dtodx1*lsf*lx1*x1FD_i.d)/(list->Rmass);
            prob_iterate_x1(list, pflux, pG);
            /* Update reduced mass */
            list->Rmass -= dtodx1*lsf*lx1*x1FD_i.d;
        }
    }
#endif /* MCTRACERS */
  ...
  for (n=0; n<NSCALARS; n++)
    ...
    if (list->Rmass > 0) {
        if (dtodx2*rx2*x2FD_jp1.d < 0) {
            /* calculate probabiliy for mc tracer transfer */
            pflux = (dtodx2*rx2*x2FD_jp1.d)/(list->Rmass);
            prob_iterate_x1(list, pflux, pG);
            /* Update reduced mass */
            list->Rmass += dtodx2*rx2*x2FD_jp1.d;
        }
        /* If outgoing (left face) flux, MC iterate through tracers */
        if (dtodx2*lx2*x2FD_j.d > 0) {
            /* calculate probabiliy for mc tracer transfer */
            pflux = (dtodx2*lx2*x2FD_j.d)/(list->Rmass);
            prob_iterate_x1(list, pflux, pG);
            /* Update reduced mass */
            list->Rmass -= (dtodx2*lx2*x2FD_j.d);
        }
    }
#endif /* MCTRACERS */
  ...
  for (n=0; n<NSCALARS; n++)
    ...
    if (list->Rmass > 0) {
        if (dtodx3*rx3*x3FD_kp1.d < 0) {
            /* calculate probabiliy for mc tracer transfer */
            pflux = (dtodx3*rx3*x3FD_kp1.d)/(list->Rmass);
            prob_iterate_x1(list, pflux, pG);
            /* Update reduced mass */
            list->Rmass += dtodx3*rx3*x3FD_kp1.d;
        }
        /* If outgoing (left face) flux, MC iterate through tracers */
        if (dtodx3*lx3*x3FD_k.d > 0) {
            /* calculate probabiliy for mc tracer transfer */
            pflux = (dtodx3*lx3*x3FD_k.d)/(list->Rmass);
            prob_iterate_x1(list, pflux, pG);
            /* Update reduced mass */
            list->Rmass -= (dtodx3*lx3*x3FD_k.d);
        }
    }
#endif /* MCTRACERS */
