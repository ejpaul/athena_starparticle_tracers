...
static Real rho_small,M_GMC,eps_GMC,R_GMC,kappa_IR,Psi,psi,v_final,eps_perturb;
  ...
  for (k=kl; k<=ku; k++) {
    for (j=jl; j<=ju; j++) {
      for (i=il; i<=iu; i++) {
        ...
        // pG->U[k][j][i].d = rho(rijk),rho_small);
          
          pG->U[k][j][i].d = M_GMC/(FOUR_3RDS*PI*CUBE(rcloud));
      }
    }
  }
...
//   M_GMC/(FOUR_3RDS*PI*CUBE(rcloud))
//}
...
//  dkx = 2.0*PI/(pG->dx1*gnx1); /* convert k from integer */
  ...
  dump_history_enroll(hst_F_out,"\\int F_out dV");
  dump_history_enroll(hst_jsrc,"\\int jsrc dV");
#ifdef STARPARTICLES
