...
void problem(DomainS *pDomain)
{
  ...
  int iprob;
  Real n0,T0,P0,v0,r,rin,rout,n1;
  Real Psn, SNvol=0., dVol=pGrid->dx1*pGrid->dx2*pGrid->dx3;
...
#ifdef SEDOV
  Real v1r,v2r,v3r,vr,Pr,rhor,xi0=1.15167;
  Real Ush,trad,rho1,xi,rhomin=1.e-3;
...
/* Read problem parameters */
  ...
  P0    = par_getd("problem","P0")*Punit;
  n0    = par_getd("problem","n0");
  T0    = P0/1.1/n0;
  n1    = par_getd("problem","n1");
#ifdef TEST_STAR
  ton 	= par_getd("problem","ton");
#endif
...
#ifndef STAR_PARTICLE
  my_max = SNvol;
  ierr = MPI_Allreduce(&my_max, &SNvol, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#ifdef SEDOV
  my_max = Ektot;
  ierr = MPI_Allreduce(&my_max, &Ektot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  my_max = Ethtot;
  ierr = MPI_Allreduce(&my_max, &Ethtot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif
#endif
  Etot= Ektot+Ethtot;
  fth = 0.717/Ethtot;
  ...
  for (j=js; j<=je; j++) {
    for (i=is; i<=ie; i++) {
      ...
      if(r<=rin) {
        ...
        //fr= 1.28*(0.5-0.5*tanh);
        get_Sedov_Taylor(xi,&rhor,&vr,&Pr);
        rhor = MAX(rhor*rho1*(Gamma+1)/Gamma_1,rhomin);
        vr = fv*vr*2.0/(Gamma+1)*Ush;
        Pr = fth*Pr*2.0/(Gamma+1)*rho1*SQR(Ush);
        if(r<pGrid->dx1) vr = 0.0; 
        v1r = vr*(x1)/r;
        v2r = vr*(x2)/r;
        v3r = vr*(x3)/r;
...
#ifndef BAROTROPIC
        pGrid->U[k][j][i].E = (Pr/Gamma_1)+0.5*pGrid->U[k][j][i].d*(SQR(v1r) + SQR(v2r) +SQR(v3r));
#endif
      }
#else
#ifndef BAROTROPIC
      if(r<=rin) pGrid->U[k][j][i].E = Psn/Gamma_1
             + 0.5*(SQR(pGrid->U[k][j][i].M1) + SQR(pGrid->U[k][j][i].M2)
             + SQR(pGrid->U[k][j][i].M3))/pGrid->U[k][j][i].d;
...
//  GM = pGrid->units.G*1.e4;
//  StaticGravPot=GravPot;
//  if(myID_Comm_world==0) create_teststar(pGrid,1.e3/pGrid->units.Msun,0.0,0.0,0.0,0.0,0);
/*
#ifdef SHEARING_BOX
  if(myID_Comm_world==0) create_teststar(pGrid,0.1,0,3.0,1.0,1);
#else
  v0 = sqrt(GM/3.);
  printf("circular velocity = %g\n",v0);
  if(myID_Comm_world==0) create_teststar(pGrid,1.0,0,3.0,v0,1);
#endif
*/
#endif
...
#ifdef TEST_STAR
  grav_mean_rho = par_getd("problem","n0");
#else
  grav_mean_rho = 0.0;
#endif
#endif
