...
/* Constant density and temperature initially */
  ...
  for (j=js; j<=je; j++) {
    for (i=is; i<=ie; i++) {
      ...
      if(iprob == 1) pGrid->U[k][j][i].d *= (1+amp*sin(kwx*x1)); //eigen mode perturbation
      if(iprob == 2) pGrid->U[k][j][i].d *= (1+amp*(ran2(&iseed)-0.5)); //white noise
      if(iprob == 3) pGrid->U[k][j][i].d *= (1+amp*(drho[k][j][i]/std_dev)); //gaussian random field
