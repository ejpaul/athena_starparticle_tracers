...
#endif
  int nl,nd;
  Real max_v1=0.0,max_v2=0.0,max_v3=0.0,max_dti = 0.0,max_cs=0.0;
  Real tlim,old_dt;
#ifdef CYLINDRICAL
  Real x1,x2,x3;
#endif
...
#endif /* MHD */
        max_cs = MAX(max_cs,sqrt((double)asq));
...
/* compute maximum velocity with star particles */
#ifdef STAR_PARTICLE
    pList = pGrid->Lstars;
    while (pList) {
      pStar = &(pList->starpar);
      if (pGrid->Nx[0] > 1)
        max_v1 = MAX(max_v1,pStar->v1);
      if (pGrid->Nx[1] > 1)
        max_v2 = MAX(max_v2,pStar->v2);
      if (pGrid->Nx[2] > 1)
        max_v3 = MAX(max_v3,pStar->v3);
      pList = pList->next;
    }
#endif /* STAR_PARTICLE */    
...
/* Spread timestep across all Grid structures in all Domains */
  ...
  for (nl=0; nl<=(pM->NLevels)-1; nl++){
    for (nd=0; nd<=(pM->DomainsPerLevel[nl])-1; nd++){
      if (pM->Domain[nl][nd].Grid != NULL) {
#ifdef STAR_PARTICLE
        pM->Domain[nl][nd].Grid->dt_old = pM->Domain[nl][nd].Grid->dt;
#endif
  ...
  if(pM->dt < 1.e-7) ath_error("time step is too small! time=%g dt=%g v1,v2,v3,cs=%g %g %g %g\n",pM->time,pM->dt,max_v1,max_v2,max_v3,max_cs);
