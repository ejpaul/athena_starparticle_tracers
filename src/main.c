...
/* Initialize nested mesh hierarchy. */
  ...
  init_particle(&Mesh);
    ...
    /* grid initialized, tracers initialized in problem file */
    init_tracer_grid(&Mesh);
#endif
...
#ifdef STAR_PARTICLE
  /* density threshold for star particle creation */
  rho_crit = par_getd_def("problem","rho_crit",1.0);
#endif
...
#ifdef STAR_PARTICLE
#ifdef DEBUG
  printf("[DEBUG %d] init feedback... \n",myID_Comm_world);
#endif
  feedback_starparticles_init(&Mesh);
#endif
...
#ifdef SELF_GRAVITY
#ifdef DEBUG
  printf("[DEBUG %d] init self-gravity... \n",myID_Comm_world);
#endif
  SelfGrav = selfg_init(&Mesh);
  for (nl=0; nl<(Mesh.NLevels); nl++){ 
    for (nd=0; nd<(Mesh.DomainsPerLevel[nl]); nd++){  
      if (Mesh.Domain[nl][nd].Grid != NULL){
        if(ires == 0) (*SelfGrav)(&(Mesh.Domain[nl][nd]));
#ifdef DEBUG
  printf("[DEBUG %d] init bval self-gravity... \n",myID_Comm_world);
#endif
...
#ifdef DEBUG
  printf("[DEBUG %d] dump initial data... \n",myID_Comm_world);
#endif
...
#ifdef DEBUG
  printf("[DEBUG %d] integrator... \n",myID_Comm_world);
#endif
    for (nl=0; nl<(Mesh.NLevels); nl++){ 
      for (nd=0; nd<(Mesh.DomainsPerLevel[nl]); nd++){  
        if (Mesh.Domain[nl][nd].Grid != NULL){
#ifdef STAR_PARTICLE
/* moved age_starparticles here for better output treatment */
/* age is not used elsewhere */
          age_starparticles(Mesh.Domain[nl][nd].Grid);
#endif
...
#ifdef STAR_PARTICLE
#ifdef DEBUG
  printf("[DEBUG %d] integrator star particle... \n",myID_Comm_world);
#endif
...
/*--- Step 9?. ---------------------------------------------------------------*/
/* User work (defined in problem()) -- where should it be? */

  Userwork_in_loop(&Mesh);

/*--- Step 9e. ---------------------------------------------------------------*/
/* Creation of Star Particle */


#ifdef STAR_PARTICLE
#ifdef DEBUG
  printf("[DEBUG %d] starparticle creation... \n",myID_Comm_world);
#endif
    for (nl=0; nl<(Mesh.NLevels); nl++){
      for (nd=0; nd<(Mesh.DomainsPerLevel[nl]); nd++){
        if (Mesh.Domain[nl][nd].Grid != NULL){
          create_starparticles(&(Mesh.Domain[nl][nd]));
          synchro_starparticles(&(Mesh.Domain[nl][nd]));
        }
      }
    }
...
#ifdef DEBUG
  printf("[DEBUG %d] boundary condition... \n",myID_Comm_world);
#endif
    for (nl=0; nl<(Mesh.NLevels); nl++){ 
      for (nd=0; nd<(Mesh.DomainsPerLevel[nl]); nd++){  
        if (Mesh.Domain[nl][nd].Grid != NULL){
      ...
      for (nl=0; nl<(Mesh.NLevels); nl++){
          for (nd=0; nd<(Mesh.DomainsPerLevel[nl]); nd++){
              if (Mesh.Domain[nl][nd].Grid != NULL){
                  GridS *pG=(Mesh.Domain[nl][nd].Grid);
#ifdef DEBUG
                  tracer_debug(pG);
#endif
#ifdef TOPHAT
                  /* Implement top hat algorithm (must be done after bvals_mc */
                   mc_tophat(pG);
#endif
              }
          }
