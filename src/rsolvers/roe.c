...
/* For high velocity, use HLLE flux */
#ifdef MACH_NUMBER_CHECK
  if((SQR(Wl.Vx) > (MSQR*Wl.P/Wl.d)) || 
     (SQR(Wl.Vx) > (MSQR*Wr.P/Wr.d)) ||
     (SQR(Wr.Vx) > (MSQR*Wl.P/Wl.d)) ||
     (SQR(Wr.Vx) > (MSQR*Wr.P/Wr.d))) hlle_flag=3; 
  else hlle_flag=0;
#endif
 ...
 * Compute Roe flux */
  ...
  for (m=0; m<NWAVE; m++) {
//    coeff[m] = 0.5*(fabs(ev[m])+etah)*a[m]; // Sanders et al. (1998) (9a)
    coeff[m] = 0.5*MAX(fabs(ev[m]),etah)*a[m]; // Sanders et al. (1998) (9b)
//    coeff[m] = 0.5*(fabs(ev[m]) >= etah ? fabs(ev[m]) : SQR(ev[m])/4.0/eta+eta)*a[m]; // Sanders et al. (1998) (9c)
#ifdef CYLINDRICAL
