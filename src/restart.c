...
/* Read grav. potential */
      ...
      fgets(line,MAXLEN,fp); /* Read the '\n' preceeding the next string */
      fgets(line,MAXLEN,fp);
      if(strncmp(line,"POTENTIAL",9) != 0)
        ath_error("[restart_grids]: Expected POTENTIAL, found %s",line);
      for (k=ks; k<=ke; k++) {
        for (j=js; j<=je; j++) {
          for (i=is; i<=ie; i++) {
            fread(&(pG->Phi[k][j][i]),sizeof(Real),1,fp);
          }
        }
      }
#endif
...
/* count the number of particles with different types */
      ...
      fgets(line,MAXLEN,fp); /* Read the '\n' preceeding the next string */
      fgets(line,MAXLEN,fp);
      if(strncmp(line,"STAR PARTICLE LIST",18) != 0) 
//        nstars = 0;
        ath_error("[restart_grids]: Expected STAR PARTICLE LIST, found %s",line);
//      else
//      fread(&(nstars),sizeof(int),1,fp);
      fread(&(nstars),sizeof(long),1,fp);
      pG->next_starpar_id = nstars;
      
      for (m=0; m<nstars; m++) {
        /* Allocate new star particle */
        pList = (StarParListS *)calloc_1d_array(1,sizeof(StarParListS));
        if (pList == NULL)
          ath_error("[starparticle]: Error calling calloc_1d_array\n");
        
        /* Read in star particle data */
        pStar = &(pList->starpar);
        fread(&(pStar->id),sizeof(int),1,fp);
        fread(&(pStar->merge_history),sizeof(int),1,fp);
        fread(&(pStar->isnew),sizeof(int),1,fp);
        fread(&(pStar->m),sizeof(Real),1,fp);
        fread(&(pStar->x1),sizeof(Real),1,fp);
        fread(&(pStar->x2),sizeof(Real),1,fp);
        fread(&(pStar->x3),sizeof(Real),1,fp);
        fread(&(pStar->v1),sizeof(Real),1,fp);
        fread(&(pStar->v2),sizeof(Real),1,fp);
        fread(&(pStar->v3),sizeof(Real),1,fp);
        fread(&(pStar->age),sizeof(Real),1,fp);
        fread(&(pStar->mdot),sizeof(Real),1,fp);
/* used for more accurate treatment in control volume */
        fread(&(pStar->x1_old),sizeof(Real),1,fp);
        fread(&(pStar->x2_old),sizeof(Real),1,fp);
        fread(&(pStar->x3_old),sizeof(Real),1,fp);
        fread(&(pStar->mghost),sizeof(Real),1,fp);
        fread(&(pStar->M1ghost),sizeof(Real),1,fp);
        fread(&(pStar->M2ghost),sizeof(Real),1,fp);
        fread(&(pStar->M3ghost),sizeof(Real),1,fp);
/* used for feedback */
        fread(&(pStar->navg),sizeof(Real),1,fp);
        fread(&(pStar->n2avg),sizeof(Real),1,fp);
        fread(&(pStar->v1avg),sizeof(Real),1,fp);
        fread(&(pStar->v2avg),sizeof(Real),1,fp);
        fread(&(pStar->v3avg),sizeof(Real),1,fp);
        fread(&(pStar->eavg),sizeof(Real),1,fp);
        fread(&(pStar->Vol),sizeof(Real),1,fp);
        fread(&(pStar->radius),sizeof(Real),1,fp);
        
        if (starpar_ingrid(&(pM->Domain[nl][nd]),pStar))
      ...
      while(pList){
        pStar = &(pList->starpar);
        pG->next_starpar_id = MAX(pG->next_starpar_id,pStar->id+1);
        pList = pList->next;
      }
#endif /* STAR_PARTICLE */
...
void dump_restart(MeshS *pM, OutputS *pout)
{
  ...
  StarParS *pStar = NULL;
  int m,nstars;
  int ibuf_starp[NELEM_INT_STARP];
  Real rbuf_starp[NELEM_REAL_STARP];
#endif
  int bufsize, nbuf = 0;
  Real *buf = NULL;
...
/* Write init_id */
...
#ifdef STAR_PARTICLE
      fprintf(fp,"\nSTAR PARTICLE LIST\n");
      /* Write out the number of star particles */
      nstars = 0;
      pGstars = pG->Gstars;
      while (pGstars) {
        nstars++;
      ...
      fwrite(&(nstars),sizeof(long),1,fp);
      
      pGstars = pG->Gstars;
      while (pGstars) {
        pStar = &(pGstars->starpar);
        /* Write star particle int data */
        m = 0;
        ibuf_starp[m++] = pStar->id;
        ibuf_starp[m++] = pStar->merge_history;
        ibuf_starp[m++] = pStar->isnew;
        if (m == NELEM_INT_STARP)
          fwrite(ibuf_starp,sizeof(int),m,fp);
        else
          ath_error("[dump_restart]:  Star particle structure error!\n");
        
        /* Write star particle Real data */
        m = 0;
        rbuf_starp[m++] = pStar->m;
        rbuf_starp[m++] = pStar->x1;
        rbuf_starp[m++] = pStar->x2;
        rbuf_starp[m++] = pStar->x3;
        rbuf_starp[m++] = pStar->v1;
        rbuf_starp[m++] = pStar->v2;
        rbuf_starp[m++] = pStar->v3;
        rbuf_starp[m++] = pStar->age;
        rbuf_starp[m++] = pStar->mdot;
/* used for more accurate treatment in control volume */
        rbuf_starp[m++] = pStar->x1_old;
        rbuf_starp[m++] = pStar->x2_old;
        rbuf_starp[m++] = pStar->x3_old;
        rbuf_starp[m++] = pStar->mghost;
        rbuf_starp[m++] = pStar->M1ghost;
        rbuf_starp[m++] = pStar->M2ghost;
        rbuf_starp[m++] = pStar->M3ghost;
/* used for feedback */
        rbuf_starp[m++] = pStar->navg;
        rbuf_starp[m++] = pStar->n2avg;
        rbuf_starp[m++] = pStar->v1avg;
        rbuf_starp[m++] = pStar->v2avg;
        rbuf_starp[m++] = pStar->v3avg;
        rbuf_starp[m++] = pStar->eavg;
        rbuf_starp[m++] = pStar->Vol;
        rbuf_starp[m++] = pStar->radius;

        if (m == NELEM_REAL_STARP)
          fwrite(rbuf_starp,sizeof(Real),m,fp);
        else
          ath_error("[dump_restart]:  Star particle structure error!\n");
