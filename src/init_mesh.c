...
#ifdef STAR_PARTICLE
    pM->Domain[nl][nd].bcflag_ix1 = pM->BCFlag_ix1;
    pM->Domain[nl][nd].bcflag_ox1 = pM->BCFlag_ox1;
    pM->Domain[nl][nd].bcflag_ix2 = pM->BCFlag_ix2;
    pM->Domain[nl][nd].bcflag_ox2 = pM->BCFlag_ox2;
    pM->Domain[nl][nd].bcflag_ix3 = pM->BCFlag_ix3;
    pM->Domain[nl][nd].bcflag_ox3 = pM->BCFlag_ox3;
#endif /* STAR_PARTICLE */
...
#ifdef STAR_PARTICLE
  /* Create MPI structure type for passing */
  MPI_Datatype type[NELEM_STARP];
  MPI_Aint displs[NELEM_STARP];
  int blocklen[NELEM_STARP];
  StarParS starpar;
  
  /* Initialize data type.  As the star particle structure StarParS is
   * modified in athena.h, type[] must be modified here.  The order of the
   * data types must match those in the structure definition.  Also, the
   * number of each type of element in the structure (NELEM_INT_STARP and 
   * NELEM_REAL_STARP) must be modified in defs.in */
  for (i=0; i<NELEM_INT_STARP; i++) type[i] = MPI_INT;
  for (i=NELEM_INT_STARP; i<NELEM_STARP; i++) type[i] = MPI_DOUBLE;
  /* Initialize blocklen (length of any array elements in the structure) */
  for (i=0; i<NELEM_STARP; i++) blocklen[i] = 1;
  
  /* As the star particle structure StarParS is modified in athena.h, displs[]
   * must be modified here.  The order of the displacements MUST match the
   * order of elements in the structure definition! */
  i=0;
  displs[i++] = (MPI_Aint)&starpar.id    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.merge_history  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.isnew - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.m     - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.x1    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.x2    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.x3    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.v1    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.v2    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.v3    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.age   - (MPI_Aint)&starpar;
/* used for more accurate treatment in control volume N=7 */
  displs[i++] = (MPI_Aint)&starpar.mdot  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.x1_old    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.x2_old    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.x3_old    - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.mghost  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.M1ghost  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.M2ghost  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.M3ghost  - (MPI_Aint)&starpar;
/* used for feedback_starparticle.c N=8 */
  displs[i++] = (MPI_Aint)&starpar.navg  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.n2avg  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.v1avg  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.v2avg  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.v3avg  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.eavg  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.Vol  - (MPI_Aint)&starpar;
  displs[i++] = (MPI_Aint)&starpar.radius  - (MPI_Aint)&starpar;
  
  /* Create the MPI star particle structure type */
  ierr = MPI_Type_create_struct(NELEM_STARP, blocklen, displs, type,
                                  &(pD->StarParType));
  if (ierr != MPI_SUCCESS)
    ath_error("[init_mesh]: Error calling MPI_Type_create_struct!\n");

  /* Commit the MPI star particle structure type */
  ierr = MPI_Type_commit(&(pD->StarParType));
  if (ierr != MPI_SUCCESS)
