  ...
  for (nd=0; nd<pM->DomainsPerLevel[nl]; nd++){
    if (pM->Domain[nl][nd].Grid != NULL) {
      ...
      pG->sfr_ratio = NULL;
        ...
        int size = N1T*N2T*N3T*(par_geti("tracers","N_tracers"));
        pG->ntracers = size;
#endif
...
#ifdef VFTRACERS
        pG->U_prev = (ConsS***)calloc_3d_array(n3z, n2z, n1z, sizeof(ConsS));
        if (pG->U_prev == NULL) goto on_error1;
#endif /* VFTRACERS */
...
#ifdef STAR_PARTICLE
      pG->dhalf = (Real***)calloc_3d_array(n3z, n2z, n1z, sizeof(Real));
      if (pG->dhalf == NULL) goto on_error11;
#endif
#endif /* SELF_GRAVITY */
