...
void show_config(void)
{
...
#if defined(STAR_PARTICLE)
#ifdef STRICT_STARPARTICLE_CREATION
  ath_pout(0," Starparticles:           strict creation\n");
#else
  ath_pout(0," Starparticles:           simple creation\n");
#endif
#else
  ath_pout(0," Starparticles:           OFF\n");
#endif
  ...
  ath_pout(0," Frist order flux corr.:  ON\n");
#else
  ath_pout(0," Frist order flux corr.:  OFF\n");
#endif
...
#if defined(OPERATOR_SPLIT_COOLING)
  par_sets("configure","cooling","ON","operator split cooling");
#else
  par_sets("configure","cooling","OFF","operator split cooling");
#endif
...
#if defined(STAR_PARTICLE)
  #if defined(STRICT_STARPARTICLE_CREATION)
    par_sets("configure","star particles","strict","sctrict creation");
  #else
    par_sets("configure","star particles","simple","simple creation");
  #endif
#else
    par_sets("configure","star particles","none","no star particles");
#endif
...
#ifdef FIRST_ORDER_FLUX_CORRECTION
  par_sets("configure","FOFC","yes","FOFC enabled?");
#else
  par_sets("configure","FOFC","no","FOFC enabled?");
#endif
