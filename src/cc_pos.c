...
#ifdef STAR_PARTICLE
void cc_ijk(const GridS *pG, const Real x1, const Real x2, const Real x3,
            int *pi, int *pj, int *pk)
{
  *pi = (pG->Nx[0] > 1) ? (int)(pG->is + floor((x1 - pG->MinX[0])/pG->dx1)) : pG->is;
  *pj = (pG->Nx[1] > 1) ? (int)(pG->js + floor((x2 - pG->MinX[1])/pG->dx2)) : pG->js;
  *pk = (pG->Nx[2] > 1) ? (int)(pG->ks + floor((x3 - pG->MinX[2])/pG->dx3)) : pG->ks;
  return;
}
#endif /* STAR_PARTICLE */
