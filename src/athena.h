...
#error: TRACERS requires VL integrator
#endif /* CTU */
#ifdef MESH_REFINEMENT
#error: TRACERS requires NO_MESH_REFINEMENT
#endif /* SMR */
#include "tracers/tracers.h"
#endif /*TRACERS */
 ...
 * If new property is added to the structure, you need to modify:
 * 1. NELEM_INT_STARP and NELEM_REAL_STARP in defs.h.in and reconfigure
 * 2. #1 Change automatically modify NELEM_STARP
 * 3. MPI_Datatype displs[NELEM_STARP] in init_mesh.c
 * 4. Always put ints first, then Reals (alphabetical order by type)
 * 5. read and write restart dump for star particle in restart.c
 */
#ifdef STAR_PARTICLE
typedef struct StarPar_s{
  ...
  int merge_history;
  int isnew;
/* fundamental parameters NREAL=9 */
  Real m;
  Real x1;
  Real x2;
  Real x3;
  ...
  Real age;
  Real mdot;
/* used for more accurate treatment in control volume NREAL=7 */
  Real x1_old;
  Real x2_old;
  Real x3_old;
  Real mghost;
  ...
  Real M2ghost;
  Real M3ghost;
/* used for feedback_starparticle.c NREAL=8 */
  Real navg;
  Real n2avg;
  Real v1avg;
  Real v2avg;
...
typedef struct Grid_s{
...
#ifdef STAR_PARTICLE
  Real ***dhalf;                /*!< density at half time step for source term correction */
#endif
  ...
  GPCouple ***Coup;          /*!< array of gas-particle coupling */
    ...
    TracerListS ***GridLists;   /*!< array of tracers lists */
#ifdef TOPHAT
  ...
  int nLstars;               /* number of local star particles */
  int nGstars;               /* number of global star particles */
  int next_starpar_id;       /* used to create unique star particle IDs */
  Real ***sfr_ratio;
#endif /*STAR_PARTICLE */
