...
static int NGrid[3],Nx[3];
static VTK_Domain ***domain_3d=NULL;
static VTK_Domain ***domain_3d_new=NULL;
...
int main(int argc, char* argv[]){
  ...
  Nx[2] = 0;
  for(i=0; i<NGrid_x; i++)
    Nx[0] += domain_3d[0][0][i].Nx;

  for(j=0; j<NGrid_y; j++)
    Nx[1] += domain_3d[0][j][0].Ny;

  for(k=0; k<NGrid_z; k++)
    Nx[3] += domain_3d[k][0][0].Nz;

  if(dom_decomp(Nx[0],Nx[1],Nx[3],nproc,&(NGrid[0]),&(NGrid[1]),&NGrid[2]))
    join_error("Error in automatic Domain decomposition for nproc=%d\n",nproc);

  domain_3d_new = (VTK_Domain***)
    calloc_3d_array(NGrid[2], NGrid[1], NGrid[0], sizeof(VTK_Domain));

  if(domain_3d_new == NULL)
      join_error("calloc_3d_array() returned a NULL pointer\n");

  init_new_domain(); /* Read in the header information */
  ...
  if(nprocs == NGrid_z*NGrid_y*nGrid_x)
  write_restart(out_name);

  read_joined_vtk(out_name);

  write_splited_restart(nproc);
...
static void write_split_vtk(const char *out_name){
  FILE *fp_out;
...
static void init_new_domain(void){
  int i, ndat, cell_dat;
  int n,m,l;
  div_t xdiv[3];
  char line[256];
  ...
  for (i=0; i<3; i++) {
    xdiv[i] = div(Nx[i], NGrid[i]);
  }

  for(n=0; n<(NGrid[2]); n++){
  for(m=0; m<(NGrid[1]); m++){
  for(l=0; l<(NGrid[0]); l++){
    domain_3d_new[n][m][l].Nx = xdiv[0].quot;
    domain_3d_new[n][m][l].Ny = xdiv[1].quot;
    domain_3d_new[n][m][l].Nz = xdiv[2].quot;
    
    domain_3d_new[n][m][l].dx = dx;
    domain_3d_new[n][m][l].dy = dy;
    domain_3d_new[n][m][l].dz = dz;
  }}}

  for(n=0; n<(NGrid[2]); n++){
    for(m=0; m<(NGrid[1]); m++){
      domain_3d_new[n][m][0].ox = ox;
      for(l=1; l<(NGrid[0]); l++){
        domain_3d_new[n][m][l].ox = domain_3d_new[n][m][l-1].ox + 
                                    domain_3d_new[n][m][l-1].dx*domain_3d_new[n][m][l-1].Nx;
      }
    }
  }
  ...
  for(n=0; n<(NGrid[2]); n++){
    for(l=0; l<(NGrid[0]); l++){
      domain_3d_new[n][0][l].oy = oy;
      for(m=1; m<(NGrid[1]); m++){
        domain_3d_new[n][m][l].oy = domain_3d_new[n][m-1][l].oy + 
                                    domain_3d_new[n][m-1][l].dy*domain_3d_new[n][m-1][l].Ny;
      }
    }
  }
  ...
  for(m=0; m<(NGrid[1]); m++){
    for(l=0; l<(NGrid[0]); l++){
      domain_3d_new[0][m][l].oz = oz;
      for(n=1; n<(NGrid[2]); n++){
        domain_3d_new[n][m][l].oz = domain_3d_new[n-1][m][l].oz + 
                                    domain_3d_new[n-1][m][l].dz*domain_3d_new[n-1][m][l].Nz;
      }
    }
  }
